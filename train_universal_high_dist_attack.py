...
distance between to original mean embeddings
'''
...
if not os.path.isdir('CMDs'):
    os.mkdir('CMDs')
with open('CMDs/train_universal_high_dist_attack.cmd', 'a') as f:
...
for word_num, new_word in enumerate(test_words):
    ...
    # For no word added, determine original mean embedding
    if word_num == 0:
        # Compute covariance of mean embeddings
        E_original = E
    else:
        # Determine average distance of shifts to attack direction for all other words
        shift = E - E_original
